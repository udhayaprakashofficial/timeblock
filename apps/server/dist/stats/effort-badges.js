...
const LEARNING_RE = /\b(learn|learning|study|studying|course|lesson|read|reading|practice|tutorial|skill|book)\b/i;
const AI_RE = /\b(ai|a\.i\.|gpt|llm|prompt|ml|machine learning|chatgpt|claude|copilot)\b/i;
function prevDateStr(dateStr) {
    return (0, time_util_1.formatDateOnly)((0, time_util_1.addDays)((0, time_util_1.dateOnly)(dateStr), -1));
}
function daysWithCompleted(tasks) {
    const set = new Set();
    for (const t of tasks) {
        if (t.status === 'completed')
            set.add(String(t.date).slice(0, 10));
    }
    return set;
}
function daysWithMatching(tasks, re, completedOnly = true) {
    const set = new Set();
    for (const t of tasks) {
        if (completedOnly && t.status !== 'completed')
            continue;
        if (re.test(t.name ?? ''))
            set.add(String(t.date).slice(0, 10));
    }
...
function computeStreakDays(tasks, todayStr) {
...
function countRecentMatchingDays(matchingDays, todayStr, window) {
    let count = 0;
    let cursor = todayStr;
    for (let i = 0; i < window; i++) {
        if (matchingDays.has(cursor))
            count += 1;
        cursor = prevDateStr(cursor);
    }
    return count;
}
function appreciationMessage(earnedCount, streakDays, todayCompleted) {
    if (earnedCount === 0) {
 ...
 */
function buildEffortSummary(input) {
    ...
    const actualLogged = week.reduce((s, t) => s + (Number(t.actualMinutes) || 0), 0);
    const todayActual = today.reduce((s, t) => s + (Number(t.actualMinutes) || 0), 0);
    ...
    const learningToday = today.some((t) => t.status === 'completed' && LEARNING_RE.test(t.name ?? ''));
    const learningDays = countRecentMatchingDays(daysWithMatching(recent, LEARNING_RE), input.date, 5);
    const aiTaskDone = recent.some((t) => t.status === 'completed' && AI_RE.test(t.name ?? ''));
    const aiSparkEarned = aiTaskDone ||
    ...
    const badges = [
        ...
        },
        {
            ...
            challenge: 'Complete a task named with learn / study / read / practice (today).',
        ...
        },
        {
            ...
            title: 'AI Spark',
            category: 'insight',
            description: 'Work like a coach: intentional blocks, deep focus, or AI practice.',
            challenge: 'Complete an AI-named task, log 45m+ today in Focus Zone, or lock 2+ timed slots this week.',
            earned: aiSparkEarned,
            tone: 'blue',
            progress: {
                current: aiSparkEarned
                    ? 1
                    : Math.min(1, (aiTaskDone ? 1 : 0) +
                        (todayActual >= 45 ? 1 : 0) +
...
//# sourceMappingURL=effort-badges.js.map